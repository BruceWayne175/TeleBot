...
@telebot.on(admin_cmd(pattern="cbutton"))  # pylint:disable=E0602
async def _(event):
    if Var.TG_BOT_USER_NAME_BF_HER is None or tgbot is None:
        await event.edit("need to set up a @BotFather bot for this module to work")
        return

    if Var.PRIVATE_GROUP_ID is None:
        await event.edit(
            "need to have a `PRIVATE_GROUP_ID` for this module to work"
        )
        return
    ...
    if reply_message.media is not None:
        ...
        tgbot_reply_message = await tgbot.get_messages(
            entity=Var.PRIVATE_GROUP_ID, ids=message_id_in_channel
        )
